...
export function FinanceiroPage() {
  ...
  const canCommissions = can('comissoes');
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 pb-16 space-y-6">
        ...
        >
          ...
          <div className="mt-3">
            ...
            <div className="mt-1 flex items-center gap-1.5 text-xs text-rose-600/80 font-medium">
              ...
              <span>{canCommissions ? 'Repasse aos especialistas' : 'Disponível no Plano Pro'}</span>
            </div>
          </div>
        </motion.div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden">
        ...
        {!canCommissions ? (
          <div className="p-8 text-center space-y-4 bg-gradient-to-b from-amber-50/20 to-white">
            ...
            <div className="max-w-md mx-auto space-y-1.5">
              <div className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-amber-500/10 text-amber-700 text-xs font-semibold">
                <Crown className="w-3.5 h-3.5" />
                Recurso exclusivo dos Planos Pro e Premium
              </div>
              <h3 className="text-lg font-bold font-luxury text-slate-900">
                Gestão Automatizada de Comissões
              </h3>
              <p className="text-xs text-slate-600 leading-relaxed">
                Seu plano atual <strong className="text-slate-900">{planName}</strong> não inclui o rateio automático de comissões por profissional. Faça upgrade para calcular repasses, margem líquida e relatórios detalhados por especialista.
              </p>
            </div>
            <button
              onClick={() => navigate('/plan-gate')}
              className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-slate-900 text-amber-400 text-xs font-bold hover:bg-slate-800 transition-colors shadow-xs cursor-pointer"
            >
